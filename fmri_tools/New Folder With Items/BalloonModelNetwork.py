...
def u_func(timing, u, tval):
    tv = timing-tval
    tval = timing[tv<=0]
    return u[timing==tval, :]

def neureq(t, I, params): 
    kappa1, tau, N0, A, C, timing, u, u_func = params
    # ut = u_func(timing, u, t)
    idx = np.where(timing == t)
    ut = u[idx, :]
    N = ut - C @ I
    # N = ut - I
    if np.any(N > -N0):
...
class BalloonModel():
    ...
    def StimulusToNeural(self):
        ...
        params = [self.kappa1, self.tau, self.N0, A, self.C, self.timing, self.u, u_func] # Bundle parameters for ODE solver
    ...
    def plot_signals(self, which_one='BOLD', add_events=True):
        ...
        for inode in range(self.Nnds):
            ...
            ax[-1].set_xlabel('time')
            if add_events:
                [ax[inode].axvline(x=i, color='red', alpha=0.5) for i in range(self.Nt) if self.u_intrinsic[i, inode]!= 0]